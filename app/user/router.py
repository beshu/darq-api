...
async def edit_user(
    ...
    return {
        "message": "ok",
        "user": json.loads(updated_user.json(exclude={"hashed_pass"})),
    }


@r.get("/{user_id}", response_model=User)
async def get_user(
    user_id: str, client: motor.motor_asyncio.AsyncIOMotorClient = Depends(get_client)
):
    try:
        user = await db.get_user(client=client, filter={"_id": PyObjectId(user_id)})
        return user.dict(exclude={"hashed_pass"})
    except UserIsNone:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="There is no such user with requested id",
        )
    except PyMongoError:
...
async def create_user(
    ...
    try:
        user = (
            User(**payload.dict())
            if payload.role
            else User(**payload.dict(exclude={"role"}))
        )
        user.hashed_pass = hash_password(payload.password)
        created_user = await db.create_user(client, user)
        return created_user.dict(exclude={"hashed_pass"})
